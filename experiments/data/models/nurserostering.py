...
def nurserostering_soft_hard_model(horizon, shifts: pd.DataFrame, staff, days_off,
                                   shift_on, shift_off, cover):
    """
    Hard/soft variant of :func:`nurserostering_model`, built for MCS enumeration.

    This is the SAME model as :func:`nurserostering_model`, with exactly one thing changed:
    the split between what is *enforced* and what is *relaxable*.

      * HARD (must always hold, can never be part of an MCS): every constraint that the
        original model adds with ``model += ...`` -- the structural rostering rules:
        cannot-follow, per-type max shifts, min/max total minutes, max/min consecutive
        shifts, min consecutive days off, max weekends, and fixed days-off.

      * SOFT (relaxable, candidates for an MCS): the three things the original model folded
        into its linear ``objective`` as weighted penalties -- shift-on requests, shift-off
        requests, and cover requirements. Here they are posted as ordinary Boolean
        constraints instead of penalty terms, and NO objective is created:

          - shift-on request   -> ``nurse_view[i, day] == shift``   (the request is honoured)
          - shift-off request  -> ``nurse_view[i, day] != shift``   (the shift is avoided)
          - cover requirement  -> ``Count(nurse_view[:, day], shift) == requirement``
            (posted directly, WITHOUT the ``slack_over``/``slack_under`` variables the
            original used to absorb violations into the objective -- the "softness" of the
            constraint now replaces that slack entirely)

    Returns ``(hard, soft, soft_names, nurse_view)``: two plain lists of cpmpy constraints
    (ready for :func:`cpmpy.tools.explain.marco` / ``make_assump_model``) plus a descriptive
    name per soft constraint and the decision-variable array.
    """
    n_nurses = len(staff)
    ...
    hard = []  # structural rostering rules -- always enforced
    soft = []  # shift-on / shift-off / cover -- relaxable, MCS candidates
    soft_names = []

    # ------------------------------------------------------------------ HARD --
    # (identical rules to nurserostering_model, only collected into `hard`
    #  instead of being added to a Model)

    # Shifts which cannot follow the shift on the previous day.
    for id, shift in shifts.iterrows():
        for other_shift in shift['cannot follow']:
            hard.append((nurse_view[:, :-1] == SHIFTS.index(id)).implies(
    ...
    for i, nurse in staff.iterrows():
        for shift_id, shift in shifts.iterrows():
            max_shifts = nurse[f"max_shifts_{shift_id}"]
            hard.append(cp.Count(nurse_view[i], SHIFTS.index(shift_id)) <= max_shifts)

    # Minimum and maximum amount of total time in minutes that can be assigned to each employee.
    shift_length = cp.cpm_array([0] + shifts['Length'].tolist())  # FREE = length 0
    for i, nurse in staff.iterrows():
        time_worked = cp.sum(shift_length[nurse_view[i, d]] for d in range(horizon))
        hard.append(time_worked <= nurse['MaxTotalMinutes'])
        hard.append(time_worked >= nurse['MinTotalMinutes'])
    ...
    for i, nurse in staff.iterrows():
        ...
        for d in range(horizon - max_days):
            window = nurse_view[i, d:d + max_days + 1]
            hard.append(cp.Count(window, FREE) >= 1)  # at least one holiday in this window
    ...
    for i, nurse in staff.iterrows():
        ...
        for d in range(1, horizon):
            is_start_of_working_period = (nurse_view[i, d - 1] == FREE) & (nurse_view[i, d] != FREE)
            hard.append(is_start_of_working_period.implies(cp.all(nurse_view[i, d:d + min_days] != FREE)))
    ...
    for i, nurse in staff.iterrows():
        ...
        for d in range(1, horizon):
            is_start_of_free_period = (nurse_view[i, d - 1] != FREE) & (nurse_view[i, d] == FREE)
            hard.append(is_start_of_free_period.implies(cp.all(nurse_view[i, d:d + min_days] == FREE)))

    # Max number of working weekends for each nurse
    weekends = [(i - 1, i) for i in range(1, horizon) if (i + 1) % 7 == 0]
    for i, nurse in staff.iterrows():
        n_weekends = cp.sum((nurse_view[i, sat] != FREE) | (nurse_view[i, sun] != FREE) for sat, sun in weekends)
        hard.append(n_weekends <= nurse['MaxWeekends'])

    # Days off
    for _, holiday in days_off.iterrows():
        i = (staff['ID'] == holiday['EmployeeID']).argmax()  # index of employee
        hard.append(nurse_view[i, holiday['DayIndex']] == FREE)

    # ------------------------------------------------------------------ SOFT --
    # (the three families the original model penalised in `objective`, now posted
    #  as relaxable constraints rather than weighted penalty terms)

    # Shift requests: "nurse i works shift S on day d" -- honoured iff the constraint holds.
    for _, request in shift_on.iterrows():
        i = (staff['ID'] == request['EmployeeID']).argmax()  # index of employee
        soft.append(nurse_view[i, request['Day']] == SHIFTS.index(request['ShiftID']))
        soft_names.append(f"shift_on__nurse{request['EmployeeID']}_day{request['Day']}"
                          f"_shift{request['ShiftID']}_w{request['Weight']}")

    # Shift off requests: "nurse i does NOT work shift S on day d".
    for _, request in shift_off.iterrows():
        i = (staff['ID'] == request['EmployeeID']).argmax()  # index of employee
        soft.append(nurse_view[i, request['Day']] != SHIFTS.index(request['ShiftID']))
        soft_names.append(f"shift_off__nurse{request['EmployeeID']}_day{request['Day']}"
                          f"_shift{request['ShiftID']}_w{request['Weight']}")

    # Cover constraints: exactly `Requirement` nurses on shift S on day d. Posted WITHOUT the
    # slack variables the original used -- relaxing the whole equality IS the "slack" now.
    for _, cover_request in cover.iterrows():
        nb_nurses = cp.Count(nurse_view[:, cover_request['Day']], SHIFTS.index(cover_request['ShiftID']))
        soft.append(nb_nurses == cover_request["Requirement"])
        soft_names.append(f"cover__day{cover_request['Day']}_shift{cover_request['ShiftID']}"
                          f"_req{cover_request['Requirement']}"
                          f"_wo{cover_request['Weight for over']}_wu{cover_request['Weight for under']}")

    return hard, soft, soft_names, nurse_view
